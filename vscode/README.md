...
- **Go to Definition** (F12), **Find All References** (Shift+F12), **hover**, **signature help**, and **IntelliSense** go through `RoseGoldC.exe lsp` (same process as diagnostics). Completions still trigger on `.`, `@`, and `:`. After `std.`, `math.`, `str.`, `io.`, `vec.`, `time.`, `path.`, and `json.` you get crate members. Hover shows `throws` and `@deprecated`. Snippets: `importstd`, `fromstd`, `dataimpl`.
...
- **Code actions** (lightbulb / Ctrl+.) on matching type errors: wrap a throwing call with `try`, fill missing `match` arms (cursor in the `match`), stub missing `impl Trait` methods (cursor on the type that needs them).
