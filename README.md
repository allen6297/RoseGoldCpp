...
A **framework** here is an optional stack behind `import`, not new syntax. Windowing is `import ui`. Widgets are `Label`, `Button`, `VStack`, and `HStack`. Write once: that widget tree paints a software framebuffer; win32/X11/Wayland/Cocoa blit it today, and Android/iOS/web are reserved names for the same `__ui` surface later.
