...
Short stdlib crate names rewrite to `std.*`: `import math;` is `import std.math;` (same for `str`, `io`, `vec`, `time`, `path`, `json`, `regex`, `ui`). `from vec import Vec2;` and `from std import UUID;` still pull one export. `import checks;` / `import process;` are no-ops (`checks.eq` already works).
...
fn main(): Int {
    ...
    print(std.regex.is_match("\\d+", "a42b"));
...
1. Optional: further TextField polish (soft-wrap, scroll inside multiline).
